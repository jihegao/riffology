...
def _wait(client: TestClient, run_id: str) -> dict:
    ...
    while time.monotonic() < deadline:
        response = client.get(f"/v2/projects/{PROJECT_ID}/runs/{run_id}")
        if response.status_code == 200 and response.json()["status"] in {"succeeded", "failed", "timed_out", "cancelled"}:
...
def test_v2_bootstrap_rejects_legacy_unindexed_project(tmp_path: Path) -> None:
...
def test_committed_ref_cannot_forge_human_owner_policy_without_records(tmp_path: Path) -> None:
    ...
    event_path = workspace / "projects" / PROJECT_ID / "project-events" / "00000000000000000002.json"
    event = json.loads(event_path.read_text())
    event["record_refs"][0] = {
...
def test_v2_crash_points_recover_one_receipt_one_worker_one_terminal(tmp_path: Path, crash_point: str) -> None:
    ...
    with TestClient(create_app(workspace, wind_timeout_seconds=180, owner_lease_seconds=0.25)) as client:
        assert _wait(client, RUN_ID)["status"] == "succeeded"
    project = workspace / "projects" / PROJECT_ID
...
def test_v2_dispatch_in_flight_committed_tombstone_stops_before_model_events(tmp_path: Path) -> None:
    ...
    while time.monotonic() < deadline:
        terminal = service.get_run(PROJECT_ID, RUN_ID)
...
def test_v2_late_cancel_records_exact_quantitative_outcome(
    ...
    while time.monotonic() < deadline:
        service.poll()
        metadata = service.get_run(PROJECT_ID, RUN_ID)
        if metadata["status"] in {"succeeded", "failed", "timed_out", "cancelled"}:
            break
        time.sleep(0.02)
    else:
...
def test_two_service_instances_duplicate_start_never_double_spawns(tmp_path: Path) -> None:
    ...
    try:
        ...
        _commit_cancel(workspace, prepared)
        assert first.cancel_run(PROJECT_ID, RUN_ID)["status"] == "cancelled"
    finally:
        first.shutdown()
        second.shutdown()
    states = [
...
def test_v2_terminal_replay_rejects_historical_policy_evidence_drift(tmp_path: Path) -> None:
    ...
    while time.monotonic() < deadline:
        metadata = service.get_run(PROJECT_ID, RUN_ID)
        if metadata["status"] in {"failed", "succeeded"}:
            break
        time.sleep(0.02)
    else:
...
def test_same_experiment_different_policy_has_same_behavioral_digests(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(workspace, wind_timeout_seconds=180)) as client:
        ...
        assert _wait(client, first["run_id"])["status"] == "succeeded"
        assert _start(client, second).status_code == 202
        assert _wait(client, second["run_id"])["status"] == "succeeded"
    ...
    assert first_metadata["workflow_label"] == "workflow_policy_unmet"
    assert second_metadata["workflow_label"] == "workflow_policy_met"
    assert first_metadata["policy_snapshot_digest"] != second_metadata["policy_snapshot_digest"]
    for key in ("canonical_event_sha256", "daily_kpis_semantic_sha256", "summary_semantic_sha256"):
        assert first_metadata["digests"][key] == second_metadata["digests"][key]
