...
describe("Gate 3 browser authority", () => {
  ...
  it("accepts only an exact internally consistent authoritative run admission projection", async () => { const state=project(4); state.run_admission={admissible:true,reason:"ready"}; state.projection_digest=await projectionDigest(state); const envelope={schema_id:"riff://evidence-studio/browser-projection-response/v1",schema_version:1,canonical_json_version:"riff-canonical-json-v2",project_id:state.project_id,snapshot_revision:state.snapshot_revision,projection_digest:state.projection_digest,projection:state} as const; await expect(verifyProjectionResponse(envelope)).resolves.toBe(state); for(const forgedAdmission of [{admissible:true,reason:"activation_missing"},{admissible:false,reason:"ready"},{admissible:false,reason:"unknown"},{admissible:false,reason:"activation_missing",extra:true}]){const forged=structuredClone(state) as any; forged.run_admission=forgedAdmission; forged.projection_digest=await projectionDigest(forged); await expect(verifyProjectionResponse({...envelope,projection_digest:forged.projection_digest,projection:forged})).rejects.toThrow(/Run admission/);} });
