...
for _ in $(seq 1 120); do curl --noproxy '*' -fsS 'http://[::1]:8787/health' >/dev/null 2>&1 && break; sleep 0.25; done
curl --noproxy '*' -fsS -X POST 'http://[::1]:8787/api/projects' -H 'content-type: application/json' --data '{"command_id":"11111111-1111-4111-8111-111111111111","display_name":"Playwright Wind Evidence","initial_actor":{"actor_type":"human","display_name":"E2E Owner","declared_role":"project_owner"}}' >/dev/null
