...
test("real framed project supports revision, review, 202 run, verified paged evidence, reload, and 390px 200% semantics", async ({ page }) => {
  test.setTimeout(180_000); await attachOwner(page);
  ...
  await page.setViewportSize({ width: 390, height: 844 }); await page.evaluate(() => { document.documentElement.style.zoom = "2"; }); const workbench = page.getByRole("button", { name: "Workbench" }); await expect(workbench).toBeVisible(); await workbench.click(); await expect(workbench).toHaveAttribute("aria-pressed", "true"); await expect(page.getByRole("tab", { name: "Evidence" })).toHaveAccessibleName("Evidence"); await expect(page.getByRole("table", { name: /Complete paged daily KPI rows/ })).toBeVisible();
});
