...
| **3 — Project 与执行** | 公开 Project 创建、实验计划、冻结 Runs、batch/visual、直接操作、outputs/events、scoped visual/Playwright、普通风机导入。 | 进行中。固定副本 Project、planning、batch lifecycle、cancel/recovery/cards、visual Store/supervision/dispatch 和 browser broker/frame/WebSocket 已合并；scoped Playwright、generic output/event/direct controls 和普通风机导入未完成。普通风机 diagnostic-event 验收依赖 generic event ingestion。 |
