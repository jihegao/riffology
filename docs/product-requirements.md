...
| FR-CONV-09 | OpenCode 的 developer repo-root 与普通 Product 的精确 Model/Project workspace 必须是不同的工作目录 profile；前者不得给普通 Product Agent 产品源码、任意文件或命令权限。 |
...
| NFR-OC-01 | OpenCode 启动必须使用显式、绝对且规范化的 `OPENCODE_WORKDIR`，并显式固定 `OPENCODE_EXPECTED_VERSION`（启动脚本默认 `1.18.4`）；仅当 loopback `/global/health` 返回的版本精确等于该配置且 `/path` 返回的工作目录与配置一致时才可进入可写 Agent 状态。缺失、非目录、symlink 歧义、目录不一致或版本不一致必须明确只读失败，不得回退到调用目录、其他目录或其他 provider。 |
