...
| --- | --- |
| Markdown | At most 1 MiB UTF-8 and 50,000 lines; a bounded line grammar creates fixed React elements, raw HTML stays text, and one centralized policy admits only visibly external HTTPS links. This implementation does not claim a general Markdown AST. |
...
type. The Product Vite surface sends this non-weakening development baseline:
`default-src 'none'; script-src 'self';
style-src 'self' 'unsafe-inline'; img-src 'self'; connect-src 'self';
frame-src <exact broker origin>; font-src 'self'; object-src 'none';
base-uri 'none'; form-action 'none'; frame-ancestors 'self'`.
Vite HMR is disabled so its inline bootstrap does not weaken `script-src`.
The exact platform visual-host page retains its stricter backend nonce CSP and
is the only page that redeems a broker frame; the Vite development origin is
not treated as platform authority. Any backend nonce is generated by the
backend and never derived from content. Wildcards, `unsafe-eval`, script
`unsafe-inline`, remote script/style origins, and content-derived nonces or
sources are forbidden. Any additional source requires independent security
review and a browser negative proving content cannot select it.
...
The A4-4 implementation exposes renderer DTOs rather than object bytes:

- `GET /api/models/:modelId/renderables/:fileId` and the matching
  `GET|HEAD /api/models/:modelId/files/:fileId/download`;
- `GET /api/projects/:projectId/runs/:runId/outputs/:outputId/renderable` and
  the existing checked output download;
- the existing Experiment create/update, Run start/read/cancel/trash/restore,
  output list, and diagnostic-event cursor routes through the common browser
  admission boundary; and
- `GET|HEAD /browser/projects/:projectId/runs/:runId/visual` for the exact-app
  host. Bootstrap now declares both `platformOrigin` and `brokerOrigin`.

Diagnostic event payload content never enters the browser DTO. A bounded
inspection emits only the closed payload shape, observed node count,
truncation flag, schema version, and `redacted` disposition; event sequence,
sample, type, and time remain visible. Duplicate JSON keys, duplicate CSV headers,
duplicate chart categories, invalid diagram identity, active HTML/SVG, and
over-limit content fail closed or remain opaque. Run-scoped UI requests carry
an epoch so a response from an old Run cannot overwrite the current Run.
...
- desktop shows both landmarks, while narrow/reflow-equivalent-200%-layout uses a
...
labelled reflow-equivalent-200% viewport rather than actual browser zoom.
...
### A4-4 implementation record

A4-4 implements the dynamic right pane without changing the startup/retirement
boundary:

- the renderer registry returns only bounded server-declared Markdown, code,
  table, strict JSON, chart, diagram, or opaque attachment DTOs;
- Model workspaces show declared execution/resources, honest technical state,
  on-demand technical checks, safe rendering, and attachment-only fallback;
- Project workspaces show fixed-copy identity, named editable Experiment
  configurations, deterministic samples, frozen Runs, exact public status and
  derived sample/seed/step-or-horizon/duration facts, closed resource summaries,
  checked outputs, filterable diagnostic events with
  content-free payload summaries, download, cancel, trash, and restore;
- completion cards remain platform-deterministic for eligible batch Runs, while
  visual Runs never receive one and analysis documents are never automatic;
- visual navigation uses the existing exact platform-host/broker authority.
  The Vite proxy displays the exact trusted host URL instead of attempting a
  cross-site iframe that would violate SameSite/CSP; the platform host performs
  bootstrap, bounded readiness resolution, nonce redemption, and iframe
  embedding; and
- the preinstalled wind content is exercised as an ordinary Model/Project,
  while an unrelated generic Python visual fixture proves there is no wind
  route, DTO, tab, renderer, or execution branch in Core.

Focused backend tests cover renderer limits/identity, the closed public event
payload summary, active-content download headers, owner scoping, real batch execution, visual
startup heartbeat ordering, bounded readiness resolution, and hostile broker
origin rejection. Component/client tests cover stale Run response suppression,
filter/pagination state, keyboard focus, platform-host derivation, safe
rendering, and exact CSP. The dedicated Chromium scenario passes 1/1 and covers
the ordinary Model and fixed-copy Project, real bounded batch, outputs/events/
download, generic visual process, exact platform host, restricted broker
iframe, cancel, narrow layout, keyboard, reflow-equivalent 200% layout, and
zero unexpected console errors.

The completed A4-4 branch gate records backend 592 total / 591 passed / zero
failed / one optional installed-OpenCode smoke skipped; Web 127/127; network
entry 1/1; production build; dedicated A4-4 Chromium 1/1; retained A4-2 and
A4-3 Chromium 1/1 each; retained full Chromium 15/15; independent
Visual-Agent Chromium 6/6; and the 27-document governance check plus
