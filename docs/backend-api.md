...
| `GET /api/conversations/{conversationId}/actions` | Return redacted skill/action cards without rationale, intent, affected resources, or raw tool payloads. A committed explicit `model_files_mutate` action may include only a verified sanitized `direct_apply` receipt with receipt/workspace digests, commit time, and logical Model-relative file paths plus prior/proposed digests; proposal/change-set actions, other committed actions, nonterminal actions, missing receipts, and invalid receipt bindings omit it. |
