...
test("real backend↔Mesa Gate 2 preserves revision, policy, run, event, artifact, cancellation and restart evidence", async (t) => {
  ...
  const unactivated = await fetch(`${origin}${projectPath}/runs`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(command({ experiment_revision_id: experimentId })) }); assert.equal(unactivated.status, 409, await unactivated.text()); assert.equal(existsSync(join(workspace, "projects", projectId, "run-intents")), false);
  const candidate = await fetch(`${origin}${projectPath}/wind/framed-candidate`).then(json); result = await send(`${projectPath}/wind/framed-evidence/activate`, command({ source_model_revision_id: active.model_revision_id, source_brief_revision_id: briefId, source_alignment_map_revision_id: alignmentId, source_experiment_revision_id: experimentId, expected_candidate_descriptor_digest: candidate.descriptor_digest })); assert.equal(result.status, "ready"); revision = result.snapshot_revision; const activated = await fetch(`${origin}${projectPath}/snapshot`).then(json); alignmentId = activated.current.alignment_map_revision_id; experimentId = activated.current.experiment_revision_id;
