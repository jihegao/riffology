...
test("fresh product storage initializes with durable SQLite policy and survives restart", () => {
  ...
  try {
    ...
    assert.equal((database.prepare("SELECT version FROM product_schema WHERE singleton = 1").get() as { version: number }).version, 8);
...
test("schema migrations advance sequentially from v1 through v8 and expose Agent and execution records", () => {
...
test("ordered v2 through v8 migration locks legacy providers and preserves execution migration atomicity", () => {
  ...
  try {
    ...
    assert.equal((legacy.prepare("PRAGMA user_version").get() as { user_version: number }).user_version, 8);
