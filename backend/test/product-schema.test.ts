...
import { PRODUCT_SCHEMA_VERSION, type CreateProjectFromModelInput } from "../src/product-domain.ts";
import {
...
test("ordered v2 through v13 migration locks legacy providers and preserves execution migration atomicity", () => {
  ...
  try {
    ...
    assert.equal((legacy.prepare("PRAGMA user_version").get() as { user_version: number }).user_version, PRODUCT_SCHEMA_VERSION);
