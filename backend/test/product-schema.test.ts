...
test("schema migrations advance sequentially from v1 through v9 and expose Agent and execution records", () => {
...
test("ordered v2 through v9 migration locks legacy providers and preserves execution migration atomicity", () => {
  ...
  try {
    ...
    assert.equal((legacy.prepare("PRAGMA user_version").get() as { user_version: number }).user_version, 9);
