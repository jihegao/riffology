...
export const verifyFramedWindBundle = (workspaceRoot: string, projectId: string, options: { captured_activation_id?: string; model_revision_id?: string } = {}): VerifiedFramedBundle => {
  ...
  exact(execution, ["schema_id", "schema_version", "canonical_json_version", "type", "additionalProperties", "required", "properties", "invariants"]);
  if (execution.schema_id !== "riff://wind-turbine-maintenance/execution-fields/v1" || execution.schema_version !== 1 || execution.canonical_json_version !== "riff-canonical-json-v2" || execution.type !== "object" || execution.additionalProperties !== false || canonicalJsonV2(execution.required).toString() !== canonicalJsonV2(["horizon_days", "warmup_days", "seed"]).toString() || canonicalJsonV2(execution.invariants).toString() !== canonicalJsonV2([{ rule: "warmup_days < horizon_days" }]).toString() || !plain(execution.properties)) fail("The framed execution schema metadata is invalid.");
  const executionRanges = { horizon_days: [1, 3660, "day"], warmup_days: [0, 3659, "day"], seed: [-2147483648, 2147483647, "signed_int32"] } as const;
  if (Object.keys(execution.properties).sort().join() !== Object.keys(executionRanges).sort().join()) fail("The framed execution schema fields are invalid.");
  for (const [name, [minimum, maximum, unit]] of Object.entries(executionRanges)) { const property = execution.properties[name]; exact(property, ["type", "minimum", "maximum", "unit"]); if (property.type !== "integer" || property.minimum !== minimum || property.maximum !== maximum || property.unit !== unit) fail("A framed execution field is invalid."); }
  if (preset.preset_id !== "wind-turbine-maintenance-demo-v1" || preset.executable !== true) fail("The framed preset identity is invalid.");
  const executionDefaults = { horizon_days: preset.horizon_days, warmup_days: preset.warmup_days, seed: preset.seed };
  for (const [name, [minimum, maximum]] of Object.entries(executionRanges)) { const value = executionDefaults[name as keyof typeof executionDefaults]; if (!Number.isSafeInteger(value) || value < minimum || value > maximum) fail("A framed preset execution value is invalid."); }
  if (executionDefaults.warmup_days >= executionDefaults.horizon_days) fail("The framed preset execution invariant is invalid.");
