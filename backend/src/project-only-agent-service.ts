...
const DESIGN_PREVIEW_REQUEST = /(?:预览|查看|打开|展示).{0,24}(?:模型设计|设计方案|设计文档)|(?:模型设计|设计方案|设计文档).{0,24}(?:预览|查看|打开|展示)/u;
const DESIGN_CHANGE_COMMAND = /(?:生成|创建|编写|增加|添加|修改|改动|调整|删除|重写|实现|修复|补充)/u;
...
export const projectCommandIntent = (text: string): ProjectCommandIntent => {
  ...
  const residual = stopVisual
    ...
    : text.trim();
  const designPreviewOnly = DESIGN_PREVIEW_REQUEST.test(text) && !DESIGN_CHANGE_COMMAND.test(text);
  const explicitMutation = !designPreviewOnly && EXPLICIT_PROJECT_MUTATION.test(text);
