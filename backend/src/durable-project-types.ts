...
export type LocalRunTerminalEvidence = { schema_version: 1; canonical_json_version: "riff-canonical-json-v2"; local_terminal_evidence_digest: string; project_id: string; run_id: string; terminal_status: "cancelled" | "failed"; outcome_code: "cancelled_before_dispatch" | "pre_receipt_admission_failed"; run_intent_digest: string; run_admission_digest: string; policy_snapshot_digest: string; downstream_idempotency_key: string; downstream_request_digest: string; cancel_tombstone_digest: string | null; evidence_base_snapshot_revision: number; evidence_base_project_event_digest: string; mesa_receipt_absent: true; dispatch_owner_absent: true; failure: null | { code: string; safe_message: string }; created_at: string };
...
export type VerifiedMesaArtifact = { artifact_id: string; name: "request.json" | "metadata.json" | "daily-kpis.csv" | "domain-events.jsonl" | "summary.json" | "replay-manifest.json" | "derived-views-manifest.json" | "run.log"; sha256: string };
export type VerifiedMesaTerminalMetadata = { schema_version: 1; canonical_json_version: "riff-canonical-json-v2"; terminal_metadata_digest: string; project_id: string; run_id: string; status: "succeeded" | "failed" | "timed_out" | "cancelled"; receipt_digest: string; run_intent_digest: string; run_admission_digest: string; policy_snapshot_digest: string; experiment_revision_id: string; experiment_sha256: string; artifacts: VerifiedMesaArtifact[] };
export type VerifiedMesaRunEvidence = { receipt: VerifiedMesaRunReceipt; lifecycle_records: VerifiedMesaLifecycleHead[]; terminal_metadata: VerifiedMesaTerminalMetadata | null };
